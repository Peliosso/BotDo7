...
$autorizados = [7926471341]; // IDs autorizados
...
if (isset($update['callback_query'])) {
    ...
    switch ($d) {
        case "cpf":
            editMessage($id, $mid, "📌 *Consulta CPF*\n\nEnvie assim:\n`/cpf 28536726890`", $botao_voltar);
            break;
        case "nome":
            editMessage($id, $mid, "📌 *Consulta Nome*\n\nEnvie assim:\n`/nome jair messias bolsonaro`", $botao_voltar);
            break;
        case "tel":
            editMessage($id, $mid, "📌 *Consulta Telefone*\n\nEnvie assim:\n`/tel 31975037371`", $botao_voltar);
            break;
        case "placa":
            editMessage($id, $mid, "🚧 Consulta de placa está em manutenção", $botao_voltar);
            break;
        case "planos":
            editMessage($id, $mid, "💸 *Plano Vitalício*\n\n✔️ Acesso total\n💵 R\$50,00\n📩 Fale com @RibeiroDo171", $botao_voltar);
            break;
        case "exemplos":
            editMessage($id, $mid, "📌 *Exemplos de uso:*\n\n`/cpf 28536726890`\n`/tel 11999999999`\n`/nome maria aparecida`\n\nClique em ⬅️ *Voltar* para retornar ao menu.", $botao_voltar);
            break;
        case "voltar":
            editMessage($id, $mid, "👋 *Bem-vindo ao Consultas do 7*\n\nEscolha abaixo o tipo de consulta desejada:", $menu);
            break;
        case "apagar":
...
if (stripos($texto, "/cpf ") === 0) {
    ...
    $aguarde = sendMessage($cid, "⏳ *Consultando CPF...*");
    sleep(3);
    ...
    if (!$d) {
        editMessage($cid, $aguarde['message_id'], "❌ CPF não encontrado.");
        exit;
    }
    ...
    editMessage($cid, $aguarde['message_id'], $txt, $botao_painel);
    exit;
}
...
if (stripos($texto, "/nome ") === 0) {
    ...
    if (!$r) {
        editMessage($cid, $aguarde['message_id'], "❌ Nome não encontrado.");
        exit;
    }
...
if (stripos($texto, "/tel ") === 0) {
    ...
    if (!$info) {
        editMessage($cid, $aguarde['message_id'], "❌ Telefone não encontrado.");
        exit;
    }
